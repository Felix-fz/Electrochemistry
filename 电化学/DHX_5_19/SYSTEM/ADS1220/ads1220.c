...
void ADS1220_Channel_Read_Data(void)
{
	...
	for(i=0;i<3;i++)
	{
			//while(ADC_DRDY == 0);  //数据就绪
			ads1220_ch1_voltage_value+=(2*ADS1220_Get_Voltage_Conversion_Data()-2.5)*10000;
			delay_ms(2);
		
		}
		if(ads1220_ch1_voltage_value > 0)
		{
			printf("P%05d",(int)(ads1220_ch1_voltage_value/3));
		}
		else
		{
			printf("N%05d",(int)(-ads1220_ch1_voltage_value/3));
		}
		//ADS1220_Setting(2);
		//printf("%lf\r\n",ads1220_ch1_voltage_value);
	  //printf("ch1:  %lf\r\n",ads1220_ch1_voltage_value);
	}
	else if(ads1220_cur_ch == 2)   //当前通道为channel 2
	{
		ads1220_cur_ch = 1;
		ads1220_ch2_voltage_value = (2*ADS1220_Get_Voltage_Conversion_Data()-2.5)*10000;
		ADS1220_Setting(1);				
